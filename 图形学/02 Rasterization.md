...
在经过MVP和视图变换后物体和摄像机被摆放在了正确的位置并展成了屏幕空间大小，下一步就是把物体画出来，这就是rasterization要做的事情， 屏幕就是光栅成像设备，所以光栅化和pixel有关
...
**问：除了叉乘还有其他办法判定点在三角形中吗？**

答：有的，还可以计算面积或使用重心坐标



### 3.1 Bounding Box(包围盒)

如果到此结束，那么每个三角形都要遍历一遍整个屏幕空间，为了降低这种巨大的时间和性能开销，包围盒这一概念随即出现

#### 3.1.1 Axis-aligned Bounding Box(AABB)
...
## 6. Shadow Mapping(阴影贴图)

**存在问题：**走样

**重要思想：**一个点不在阴影里说明从光源和从摄像机出发都能看到这个点

**局限**：只能处理点光源（有明显边界的硬阴影）





### 6.1 Render from Light

为了知道一个点能否被光源所看到，最直观的方法就是站在光源的角度去看这些点，做一次光栅化记录看到的任何点的深度

<img src="Images\image-20240217002347510.png" alt="image-20240217002347510" style="zoom:67%;" />

### 6.2 Project to Light

从真正的角色视角或摄像机出发来看场景，计算光源到该点的深度 $D_0$ 。对于看到的点将其投影回Render from Light对应的位置，得到当时计算的深度 $D_1$​ ，之后比较两个值的大小，如果大小相同，说明该点被光照射，如果大小不同，说明此处应该为阴影区域



通过这种方法得到一张阴影贴图用于后续Shading使用

- **深度相同**

<img src="Images\image-20240217004822261.png" alt="image-20240217004822261" style="zoom:67%;" />

- **深度不同**

<img src="Images\image-20240217005258079.png" alt="image-20240217005258079" style="zoom:67%;" />

如下图所示，本应该没有阴影的地方也看到有些计算出了阴影，这是因为**浮点数本身有精度问题**，判断相等很困难导致的，其中一个解决方法是加一个bias，但是无法从根本上完全解决

<img src="Images\image-20240217010927427.png" alt="image-20240217010927427" style="zoom:67%;" />

****

**Shadow Mapping的现有问题：**

<img src="Images\image-20240217011744596.png" alt="image-20240217011744596" style="zoom:67%;" />

即使Shadow Mapping仍然有很多还未解决的问题，但是现在工业界仍然大量应用该方法



**硬阴影 VS. 软阴影：**

<img src="Images\image-20240217012104323.png" alt="image-20240217012104323" style="zoom:67%;" />

为了实现更加真实的阴影，需要**Ray Tracing**的知识
